...
| **Scope** | England only |
| **Status** | In build — §0–§4 complete, 5.1–5.2 done (as at 2026-08-01) |
...
**Status key:** `DONE` · `NEXT` · `PLANNED` · `CONTINUOUS` · `BLOCKED` · `DEFERRED` · `CUT` — update this column as work completes.

> **Progress as at 2026-07-26.** 15 of 27 buildable steps are `DONE`, with committed code, passing tests and a measured result. Gates **DG1** and **DG3** are cleared. Two steps are `BLOCKED` on human action: 0.3 (NHS API key — off the critical path) and **1.3 (nightly cron — needs a GitHub remote, and this one costs irrecoverable history every night it waits)**. Live detail lives in `CLAUDE_PROGRESS.md`.
